...
用法：
    python tools/extract-cash-items.py   # 先跑這個
    python tools/build-fashion-data.py

輸入 reference-data/cash-items.json + name-tables/Item.json（+ name-supplement 補漏）
輸出 src/data/generated/fashion-catalog.json：
    { "meta": {...}, "items": [ { "i": id, "n": 名稱, "s": 部位, "g": 性別 }, ... ] }
...
def main():
    ...
    print(f"時裝 catalog：{len(items)} 件（無中文名 {unnamed} 件）")
    print("部位分布：", dict(sorted(by_slot.items(), key=lambda kv: -kv[1])))
    print("性別不明：", sum(1 for r in items if r["g"] == -1))

    out = {
        "meta": {
            "source": "reference-data/cash-items.json + name-tables/Item.json",
