...
- `src/nav.ts`：選單設定。**2026-08-22 改成兩層、2026-08-23 再開頂層單項**：頂層清單是 `navTree`，每個元素可以是**下拉群組**（`NavGroup`，有 `items`）或**直接連過去的單項**（`NavItem`），用 `isNavGroup()` 分辨。目前排法＝「查詢」（怪物掉落／地圖／技能／任務）、「工具」（技能配點／BOSS 計時器）、**轉蛋模擬**、**時裝搭配**。後兩個刻意放頂層：這兩個是玩起來停不下來的主打功能，收進下拉等於多一層點擊、也少了曝光。分組的理由：任務查詢在語意上就是查詢，留在頂層會讓「查詢」這個分類名說謊；而操作型工具剛好湊成一組，之後的裝備模擬器、楓幣計算機也有地方放。桌機版群組點了展開下拉、手機版側邊選單直接分組攤平（少一層點擊），頂層單項在手機版沒有分組小標。`navItems` 是攤平後的清單（含頂層單項），給不分組的地方用。**桌機主列的頂層單項不畫圖示**（`showIcon={false}`）——同一排另外兩項是群組、本來就沒圖示，只有兩項有會看起來歪掉；下拉裡與手機版仍然有圖示。**注意 nav 的圖示不是讀這裡的 emoji**，是 `site-shell.tsx` 的 `NAV_ICONS` 這份 lucide 元件對照表，新增工具兩邊都要加。
