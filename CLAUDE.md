...
11. `python tools/extract-skill.py` → `reference-data/skill.json`（技能的等級上限、前置技能、每級數值，640 個）
12. `python tools/build-skill-data.py` → `src/data/generated/skills.json`（技能查詢與配點模擬用；316 個技能＝1轉29／2轉81／3轉89／4轉117）
13. `python tools/extract-skill-icons.py` 依 `reference-data/skill-icon-map.json` 從圖集切出 `public/icons/skill/<skillId>.png`
...
- `src/nav.ts`：選單設定。**2026-08-22 改成兩層**：頂層是「查詢」（怪物掉落／地圖／技能／任務）與「工具」（技能配點／BOSS 計時器／時裝搭配）兩個主選單 `navGroups`，桌機版點了展開下拉、手機版側邊選單直接分組攤平（少一層點擊）。`navItems` 是攤平後的清單，給不分組的地方用。分兩組的理由：任務查詢在語意上就是查詢，留在頂層會讓「查詢」這個分類名說謊；而操作型工具剛好湊成第二組，之後的裝備模擬器、楓幣計算機也有地方放。**注意 nav 的圖示不是讀這裡的 emoji**，是 `site-shell.tsx` 的 `NAV_ICONS` 這份 lucide 元件對照表，新增工具兩邊都要加。
...
- `/skills` **技能查詢**（2026-08-22 上線）：吃真資料（`src/data/skills.ts` → `generated/skills.json`）。分「現行版本／未來視」——**現行 = 1 轉 + 2 轉共 110 個技能，3 轉尚未開放所以跟 4 轉一起進未來視**（tier 常數寫死在 `build-skill-data.py`，遊戲開 3 轉改一行）。**新手技能（職業 000）整個不收**，那 20 個幾乎都是活動／特殊技能（怪物騎乘、宇宙船、肥肥的弱點攻擊…）。左欄列職業、點開才展開該職業技能；**搜尋只吃技能名與職業名，不吃敘述**（吃敘述的話「攻擊」會命中三百個），搜尋時左欄仍是職業清單、沒命中的灰掉、命中的就地展開（切換成兩種清單會讓畫面跳來跳去）。右詳情：前置技能 chip（可點跳過去）→ **等級滑桿，只顯示該級敘述**→「看全部等級」才開表格（等級｜數值｜效果，點列可跳等級）。30 行純文字一次攤開對閱讀是負擔，所以預設收著。數值只顯示白名單 9 欄（MP/HP 消耗、傷害、攻擊次數/怪數、持續、冷卻、發動率、熟練度），其餘不顯示——LevelDesc 的中文敘述本來就把數字寫進去了。4 轉技能標「等級上限 N（需精通書，初始 M）」。
- `/skill-build` **技能配點模擬**（2026-08-22 上線）：選 12 個 2 轉職業之一 → 同時攤開 1 轉 + 2 轉技能，卡片格狀（圖示＋名稱＋前置徽章＋當前等級敘述＋加減）。**SP 是累計共用池不是各轉獨立**（1 轉沒點完會留到 2 轉、2 轉後也能回頭加 1 轉技能），所以預算用「到某等級為止累計可得的 SP」表示：法師 30 級前 67／70 級前 188，其他職業 61／182（公式 = 每升 1 級 +3、每次轉職 +1，四個數字都對得起來）。**兩階段並排顯示、不做切換**，配 2 轉時還看得到 1 轉餘額。嚴格模式：超額或前置不足時按鈕 disabled，**但一定附上原因**（純 disabled 不說原因會讓人反覆亂點卻不知道卡在哪）；降級也擋（把前置降到不足會讓已點技能違規）。localStorage 存草稿 + 「分享」把配點編進網址 `?j=110&b=1100000-20.1100002-30`。**不做**：新手技能、3/4 轉配點、角色等級輸入框。
