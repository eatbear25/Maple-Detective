...
14. `python tools/fetch-skill-icons.py` 補還沒有圖的技能（3/4 轉靠這支），從參考站 `data.js` 依「職業＋技能名」抓圖，同時把來源寫回 `skill-icon-map.json`
