...
9. `python tools/build-quest-data.py` → `src/data/generated/quests.json`（任務查詢頁用：任務、任務鏈、接取/完成條件、獎勵、任務道具名稱/說明、NPC 所在地圖與世界地圖點位）
   9b. `python tools/check-quest-data.py`（只讀，**每次重跑 9 之後都要跑**）把輸出逐筆對回客戶端原始資料，並列出 Check/Act 每個欄位「有用到／刻意不用」。出現 UNKNOWN 欄位 = 遊戲更新加了東西，要人去看
...
**職業中文名寫死在 `tools/jobs.py` 的 `JOBS`**（客戶端 String 表沒有職業名），技能頁與任務頁的職業限制吃同一份，只該有一份。發現跟遊戲內不符就改那裡，再重跑 `build-skill-data.py` 與 `build-quest-data.py`，已修正：110 = **狂戰士**（原本誤植「戰士」，2026-08-22）。`jobs.py` 另外收了**職業 bitmask 的解法**：任務獎勵 `Act["1"].item[].job` 是 bitmask 不是職業 id，**bit 編號 % 10 = 職業系列、// 10 = 轉職階段**（19 種 mask 全部吻合）。
...
**任務資料的八個坑（2026-08-19 逆向完成、2026-08-23 全面複查一次，別重踩）**：
...
3. **Act 有兩段，獎勵只能讀 `["1"]`**（2026-08-23 修，玩家回報）：`["0"]` 是接任務當下、`["1"]` 是完成當下。`["0"]` 的道具是「NPC 先把任務道具塞給你」（63 個任務有，例：#1034 美味的蘑菇糖果，妮娜先給 10 顆去跑腿，完成時 `["1"]` 再 `-10` 收回、另給 2 顆能吃的），兩段混著讀就會印成「獎勵 10 顆＋2 顆」。這些先給的道具存成 `give`，詳情面板獨立一段「接任務時拿到」（順便解掉「需求道具有一半不用自己找」的困惑）。負數 `money`（＝任務向玩家收費）則**兩段都要看**——#2029 瑪帕的請託是接任務當下先收 1000。
4. **`Check["0"].quest` 的 `state` 決定是前置還是互斥**（2026-08-23 修）：state 2 = 那個任務要已完成、1 = 要正在進行中（都是真前置），**0 或沒寫 = 那個任務「不能接過」＝互斥**。全部當前置的話，五條「XX 之路」轉職任務會互相變成對方的前置（「要先完成盜賊之路才能接戰士之路」），連任務鏈都被串成一條 6 步。現在拆成 `prereq` 與 `exclude`，任務鏈只吃 prereq，UI 在接取條件印「接過這些就不能接」、鏈上的互斥步驟掛「擇一」徽章。
5. **稱號勳章的獎勵不在 `Act`，在 `QuestInfo.viewMedalItem`**（24 個任務，2026-08-23 玩家回報才發現）。少讀這一欄的話，15 個「稱號挑戰／捐獻王／冒險家」會因為 Act 全空被印成「獎勵在伺服器端查不到」，但勳章其實客戶端寫得好好的（另外 9 個 Act 裡也有同一面，要去重）。**教訓：QuestInfo 不是只有 area/order 可用**，`check-quest-data.py` 已經把 QuestInfo 的 23 個欄位也納入涵蓋率檢查。
6. **完成條件不只有道具**：`Check["1"].mob` 有 79 個任務要打怪（`kill`），以前整段沒顯示、還印成「不需要繳交道具」。技能獎勵 `Act["1"].skill` 有 40 個（多半是 4 轉精通書），以前只印一顆「技能」chip 沒有名字；`masterLevel` = 把上限拉到幾級、`skillLevel` = 直接學到幾級，同一個技能常有冒險家/騎士團/龍魔導士三份 id，要用「名字＋職業」去重。
7. **大部分任務的獎勵不在客戶端**：`Act` 只有 **15 個任務**帶 `money`（11 正、4 負），**84 個任務的 `Act` 完全是空的**。原因查明：`Check` 裡有 **66 個任務**帶 `startscript`/`endscript`，代表接受/完成處理交給**伺服器端腳本**，獎勵不走 Act——例如 #2156「可以許願的彩虹鍋牛殼」有 `endscript: q2156e`，實際會給 3 萬楓幣，客戶端一個欄位都沒有。其中 **52 個是「有 script 且 Act 全空」**，獎勵一定查不到（含全部轉職任務）。掃過 `Act` 全部欄位、任務說明文字、`ScriptString` 的 quest0/quest1，客戶端都沒有腳本內容——跟掉落機率一樣只能靠玩家回報。人工補充走 **`reference-data/quest-supplement.json`**（`{ questId: { exp?, money?, cost?, pop?, items? } }`，會覆蓋客戶端資料），網站上吃到補充的任務會標「＊部分獎勵來自玩家回報補充」，查不到獎勵的則直接說明是伺服器端腳本、不會印成「沒有獎勵」。另外**負數 money 是「任務向玩家收費」不是獎勵**（例：#6000 收 100 萬），build 腳本拆成 `money`(得到) 與 `cost`(花費)。
8. **任務的實裝判斷 = 該任務的 NPC 有沒有站在「Map.json 有名字」的地圖上**。實測 490 個任務裡：NPC 全部實裝 296、部分實裝 28、完全沒有 166。定案是 296+28 算現行版本，只有 166 掛未實裝徽章。另外 155 個任務 NPC 只有 97 個查得到地圖，查不到的 14 個正常號段 NPC 是腳本動態生成的劇情 NPC（705 張地圖已確認是全部，不是漏掃）。
...
- `src/nav.ts`：選單設定。**2026-08-22 改成兩層、2026-08-23 再開頂層單項並依 GA 重排**：頂層清單是 `navTree`，每個元素可以是**下拉群組**（`NavGroup`，有 `items`）或**直接連過去的單項**（`NavItem`），用 `isNavGroup()` 分辨。目前排法＝**怪物掉落**、**BOSS 計時器**、「查詢」（地圖／技能／任務）、「模擬器」（轉蛋模擬／時裝搭配／技能配點模擬）。**頂層位置的分配標準是「觸及率 ÷ 停留時間」，不是「哪個功能最得意」**——點擊成本會乘上造訪次數，但會被停留時間稀釋：一頁進去待 100 秒，多點一下沒感覺；進去待 22 秒，多點一下就佔了 10% 的體感時間。依 2026-08-23 的 GA（141 位活躍使用者）：monsters 76.6%／58 秒、boss-timer 35.5%／22 秒、fashion 20.6%／98 秒、map 19.2%／56 秒，所以高頻的怪物掉落與「開啟→掃一眼→關掉」的 BOSS 計時器放頂層直達；時裝搭配原本放頂層的理由（主打功能）被數據推翻——只有 20% 的人碰、但碰的人待 98 秒，是深度玩具不是入口，收進下拉的代價接近 0。「模擬器」群組取代舊的「工具」：舊分組把 BOSS 計時器（查一下就走）跟技能配點（待很久）綁在一起，性質相反；現在三個成員都是「進去待很久、一次玩完」的深度工具，之後的裝備模擬器、楓幣計算機也放這裡。**quests／skills／skill-build／gacha 在那份 GA 之後才上線、還沒有數據**，目前是依性質歸類，要回頭用 GA 再確認一次。桌機版群組**滑鼠移上去就展開**下拉（2026-08-23 從 click 改成 hover），點擊仍然可以切換——鍵盤與觸控裝置只有這條路；兩個實作細節別拆掉：(1) `mouseleave` 是**延遲 120ms 才關**，因為下拉是 `absolute + mt-1`、畫在 `.relative` 容器的框外，游標穿過那 4px 空隙時會先觸發 `mouseleave`，沒有緩衝選單會在半路關掉；(2) hover 處理器用 `matchMedia("(hover: hover)")` 擋掉觸控裝置，否則觸控點一下會先觸發 `mouseenter` 展開、再被 click toggle 關掉，變成點不開。滑到頂層單項上會把展開中的下拉收掉（`NavEntry` 的 `onMouseEnter`）。手機版側邊選單直接分組攤平（少一層點擊），頂層單項在手機版沒有分組小標。`navItems` 是攤平後的清單（含頂層單項），給不分組的地方用。**桌機主列四個頂層項目都有圖示**（2026-08-23 改）：原本頂層單項用 `showIcon={false}` 關掉，理由是同一排的群組沒圖示、只有單項有會看起來歪掉；現在改成**連群組也配圖示**（查詢＝`BookOpen`、模擬器＝`Gamepad2`），整排就齊了，`showIcon` 這個 prop 已移除。手機版側邊選單的群組小標仍然是純文字（底下的項目已經有圖示了，小標再放一個會跟項目的圖示對不齊）。**注意 nav 的圖示不是讀這裡的 emoji**，是 `site-shell.tsx` 的 `NAV_ICONS` 這份 lucide 元件對照表；那份表的鍵**同時吃 `NavItem.key` 與 `NavGroup.key`**，新增工具或新增群組兩邊都要加。nav 的 label 跟頁面 h1 要一致（`fashion` 曾經 nav 寫「試衣間」、頁面寫「時裝搭配」，已統一成後者）。
...
- `/quests` **任務查詢**（2026-08-19 上線）：吃真資料（`src/data/quests.ts` → `generated/quests.json`）。490 個任務，依需求等級排序、每頁 50 筆分頁（分頁列中央標該頁的等級區間，因為按等級排序＋分頁的話翻頁只能用猜的）。**搜尋只吃任務名與獎勵道具名**——不搜 NPC 名、不搜需求道具（定案：需求道具點進詳情看就好）。列表卡片是「左等級圓徽＋任務名＋NPC 名」三項，刻意不放獎勵。詳情面板由上到下是：標題（含等級區間／可重複）→**系列鏈卡片**→ NPC →**接取條件**（職業限制／互斥任務）→**完成條件**（打倒怪物＋繳交道具）→ 花費 → 獎勵（含技能獎勵與職業分組）。**客戶端有、但刻意不上畫面的兩樣**（2026-08-23 玩家看了說是雜訊，資料留在 quests.json）：`have`（接任務時身上要有的道具——20 個楓之島教學任務都寫「要有維澤特西裝」，其餘的則是前一步任務給的、任務鏈已經講過）與 `give`（接任務時 NPC 先給的道具——它存在的意義是**把它從獎勵裡挑出來**，不是拿來顯示）。**查不到獎勵時整個「獎勵」區塊不畫**（81 個任務，獎勵在伺服器端腳本）——「沒有獎勵」「獎勵在伺服器端查不到」「回報給我們補上」這三種說明都被玩家打槍過，畫面少一塊比多一句解釋乾淨。**2026-08-23 全面複查過一次**（玩家回報獎勵有錯），修掉的都寫在上面「任務資料的七個坑」，重點：獎勵不再混進接任務時拿到的道具、打怪需求終於顯示、前置與互斥分開、技能獎勵印名字、職業限定獎勵分組印（#2001 酋長蓋房子的 16 張卷軸其實只拿得到自己職業那兩張）、獎勵有 exp/楓幣時不再多印一行「沒有道具獎勵」。系列鏈用**前後關聯圖遍歷**建（96 條、最長 21 步），但**分組鍵是系列名 `parent` 而不是連通元件**——遊戲的任務關聯會跨系列相連（「泰實夫的秘密之書」最後一步的後續就是「妖精羅雯和詛咒的娃娃」第一步），純靠連通元件會把三條線併成一條、標題和「第 N/M 步」都是錯的，而且還會透過中間沒系列名的任務間接發生。搜尋框下方有**地區 chips**（帶任務數）。**2026-08-23 重做分類**：改成 `build-quest-data.py` 的 **`TOWNS` 主城清單 ＋ 三段判定**（1. `MAP_REGION_OVERRIDE` 人工指定 → 2. 地圖名開頭就是某個主城名 → 3. 世界地圖上離哪個主城的點最近），chip 從 15 個收斂成 **10 個地區＋未實裝**：楓之島 52／弓箭手村 58／魔法森林 51／勇士之村 32／墮落城市 34／維多利亞港 17／奇幻村 29／黃金海岸 9／鯨魚號 41／結婚小鎮 1／未實裝 166，**順序＝TOWNS 宣告順序（遊戲地理動線），不是任務數**（照數量排每次更新 chip 都換位置）。舊做法是「streetName 分組＋涵蓋 ≥3 個主城代碼才依主城代碼拆開」，維多利亞是拆對了，但 streetName 本身有一堆不是大地區的值，chip 就歪掉：迷霧森林（整片其實屬奇幻村）、彩虹之地（楓之島的楓葉村那半）、戰火之地（迷宮，入口分別在勇士之村與弓箭手村門口）、墮落城市地鐵、還有隱藏地圖被 ID 前綴推去「自由市場入口」（詛咒之林其實在魔法森林，靠惡靈森林任務鏈其他五步確認）。三段判定的兩個坑：**第 3 段的底圖要取大陸圖（depth 1）不是最深的子圖**——奇幻村那張子圖上只畫得下一個主城，任何點都會被判給它（迷宮入口就是這樣被吸走）；**depth 0 的世界總圖要排除**（整座大陸糊成一團）。地區存在**任務身上（`quest.region`）不是地圖身上**，而且顯示用 NPC 查不到地圖時會**退而找同一任務的其他 NPC**（例：#6101 接任務的懷玆沒實裝、交任務的那位有），所以判不出地區 ⇔ 該任務所有 NPC 都沒實裝 ⇔ 未實裝任務，chip 因此直接叫「**未實裝**」（掛望遠鏡圖示、永遠排最後）而不是看不懂的「未知」。道具**只有 icon＋名稱前兩字標籤＋數量**（前兩字是因為卷軸類 icon 全長一樣，一個任務要五張不同卷軸時完全分不出來），hover 才出道具彈窗。點獎勵道具開彈窗反查「還有哪些任務給這個」；點 NPC 地圖開世界地圖彈窗。詳情的任務 NPC 有站立圖（`public/icons/npc/<npcId>.gif`，`download-icons.py` 從 maplestory.io 抓，147 個 NPC 有 143 個抓得到）。**不做**：個人進度勾選、任務對話原文、等級篩選框。
